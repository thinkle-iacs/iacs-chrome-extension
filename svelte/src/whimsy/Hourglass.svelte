<script lang="typescript">
  ...
  function makeGrains () {
    ...
    for (let n=0; n<grainsOfSand; n++) {
      grains.push({
        id:n,x:undefined,y:0,top:true,color:`var(${randomColor()}`,
        speed : Math.random() + 1,
      });      
    }
    return grains;
  ...
  function calculateExplosion (percentage) {    
    ...
    for (let i=0; i<grains.length; i++) {      
      ...
      grains[i].x = grains[i].xi + Math.cos(angle) * explosionCounter * grains[i].speed;
      grains[i].y = grains[i].yi + Math.sin(angle) * explosionCounter * grains[i].speed + explosionCounter*explosionCounter;
      grains[i].falling = true;
    }
  }
...
<section class="hourglass" class:exploded={explosionCounter > 0}>
  {#if explosionCounter > 0}
  <div class="fire"><Fireworks></Fireworks></div>
  {/if}
  {#if letters.length}
    <div class="message">
...
<style>
  ...
  .hourglass::before {
    ...
    height: 0;
    border-style: solid;
    border-top-width: calc(var(--glass-size)*var(--height)/var(--triangle-factor));
